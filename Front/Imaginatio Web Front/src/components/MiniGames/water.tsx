...
import panelRecogerAgua from "../../assets/Recursos web media/Panel_RecogerAgua.png"
import btnGotaAgua from "../../assets/Recursos web media/btn_GotaAgua.png"
    ...
    <main class="fixed inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-black/60 backdrop-blur-sm">
      <section className="relative w-[600px] h-[600px] flex flex-col items-center justify-center p-12">
        ...
        <div className="absolute inset-0 z-0">
          <img src={panelRecogerAgua.src} alt="Panel Recoger Agua" className="w-full h-full object-contain" />
        </div>
          ...
          {gameState === "playing" && (
            <>
              <div className="flex justify-between items-center w-full px-2">
                <span class="text-black text-center font-bold bg-blue-200/80 px-3 py-1 rounded-lg border border-blue-500">
                  ⏱️ {timeLeft}s
                </span>
              ...
              <button
                onClick={handleClick}
                class="w-40 h-35 hover:scale-110 active:scale-95 transition transform-gpu drop-shadow-lg select-none p-0 bg-transparent"
              >
                <img src={btnGotaAgua.src} alt="Gota de Agua" className="w-full h-full object-contain" />
              </button>

              <p className="text-amber-950 text-xs font-black uppercase text-center tracking-tight mt-2 bg-white/30 px-4 py-1 rounded-full border border-white/40">
