...
export default function CreditsModal() {
  ...
  return (
    <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div
        className="bg-[#f5e6c8] w-[600px] max-w-full rounded-3xl border-[12px] border-[#4e341b] shadow-2xl flex flex-col items-center relative p-8 animate-in zoom-in duration-200"
      >
        {/* Botón de cierre */}
        <button
          onClick={() => isCreditsModalOpen.value = false}
          className="absolute -top-6 -right-6 w-14 h-14 bg-red-500 border-4 border-[#4e341b] rounded-full text-white font-black text-2xl flex items-center justify-center hover:bg-red-600 hover:scale-110 active:scale-90 transition-all shadow-[0_4px_0_#4e341b]"
        >
          X
        </button>

        {/* Header */}
        <div className="bg-[#4e341b] text-[#f5e6c8] w-[110%] py-4 -mt-14 mb-8 rounded-xl border-4 border-[#8B4513] shadow-lg text-center relative">
        ...
        <div className="w-full bg-[#fff9eb] border-4 border-[#8B4513] rounded-2xl p-6 text-center space-y-4 max-h-[60vh] overflow-y-auto">
